$(function() {
	...
	function playSound(x, y, quick) {
        ...
        sound.playbackRate.value = x / $('#content').width() * 2;
        sound.connect(gain);
        gain.connect(audioCtx.destination);

        var volume = y / $('#content').height() / 4;
		...
		var context = document.getElementById('content').getContext('2d'),
			centerX = event.pageX,
	 		...
	 		height = $('#content').height(),
      		radius = 30,
			hue = Math.floor(centerX / width * 360),
			saturation = Math.floor(centerY / height * 100),
			lightness = Math.floor(centerY / height * 100);
      	...
      	playSound(centerY, centerY, false);
      }	
	});
});
